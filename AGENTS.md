...
- **릴리스된 버전은 절대 덮어쓰지 않는다.** 변경은 항상 새 버전으로 릴리스한다.
- `esbuild.config.mjs`의 버전 배너는 릴리스별 `main.js`를 식별하므로 제거하지 말 것.
- 자동 Release workflow는 없다. main 머지 후 최신 main에서 직접 빌드하고, manifest version과 같은 이름의 태그 및 GitHub Release를 만든 뒤 `main.js`와 `manifest.json`만 첨부한다.
