...
- main 머지 시 `.github/workflows/release.yml`이 manifest version과 같은 이름의 태그로 릴리스를 자동 생성하고 `main.js`, `manifest.json`을 첨부한다 (릴리스 생성은 서드파티 액션 없이 `gh` CLI). 워크플로우는 버전을 올리지 않으며, 같은 버전의 릴리스가 이미 있으면 실패한다.
