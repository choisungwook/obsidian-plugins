...
- `.github/workflows/release.yml` (Release): main에 머지되면 빌드 (테스트는 PR 단계에서 이미 통과했다고 가정하고 생략) → `main.js`/`manifest.json` attestation 생성 → attestation이 공개 API의 `gh attestation verify`로 검증될 때까지 대기(최대 10회×15초) → manifest 버전과 동일한 태그(예: `0.1.0`)로 GitHub Release를 생성하고 `main.js`/`manifest.json`을 첨부한다. Obsidian 마켓이 이 태그 형식을 요구한다. 같은 버전의 릴리스가 이미 있으면 실패하므로, 모든 PR에서 `manifest.json`과 `versions.json`의 버전을 올릴 것
