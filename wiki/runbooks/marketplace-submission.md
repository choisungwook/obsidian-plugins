...
- `manifest.json`의 `version`과 **정확히 같은 이름의 태그**로 GitHub Release를 만들고, `main.js`와 `manifest.json`을 개별 자산으로 첨부. 그 외 파일(`versions.json` 등)을 자산으로 붙이면 리뷰 봇이 "extra unsupported files" 경고를 낸다 (`versions.json`은 저장소에서 읽으므로 자산 불필요). `.github/workflows/release.yml`은 version 태그 push로 실행한다.
...
- **0.1.5도 같은 에러가 발생했다** (2026-07-17). `gh attestation verify`는 성공했지만 attestation의 `sourceRepositoryRef`/`githubWorkflowRef`가 `refs/heads/main`이었다. Obsidian의 세부 attestation 검증 기준은 공개되지 않아 ref 불일치가 확정 원인이라고 단정할 수는 없다. 다만 커뮤니티 통과 레시피는 release workflow를 version 태그 push로 실행해 두 ref가 모두 `refs/tags/<version>`이므로, 0.1.6부터 같은 방식으로 정렬한다. 공개 API 검증 게이트에서도 서명·저장소에 더해 source/workflow ref가 현재 태그이고 source digest가 현재 태그 커밋인지 검사한다.

## 절차

1. `manifest.json`/`versions.json` 버전을 확인하고 main에 머지한다.
2. 최신 main에서 manifest 버전과 같은 태그를 만들고 push한다. 태그 push가 release workflow를 실행한다.
...
3. release workflow가 성공하고 attestation의 source ref가 `refs/tags/<version>`인지 확인한다.
...
- 버전 bump는 main 머지 시점이 아니라 **모든 PR 안에서** patch 단위로 수행한다. release workflow는 버전을 올리지 않으며, manifest 버전과 다른 태그나 이미 릴리스된 버전이면 실패한다. 상세 규칙은 저장소 루트 `AGENTS.md`의 "버전 관리" 절 참고.
